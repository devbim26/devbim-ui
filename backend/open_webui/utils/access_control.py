...
def has_access(
    ...
    # Публичные знания (access_control = None) доступны для чтения всем
    ...
    # Проверяем доступ для указанного типа (read или write)
    permission_access = access_control.get(type, {})
    permitted_group_ids = permission_access.get("group_ids", [])
    permitted_user_ids = permission_access.get("user_ids", [])

    # Пользователь имеет доступ если:
    # 1. Он в списке разрешенных пользователей
    # 2. Он состоит в одной из разрешенных групп
