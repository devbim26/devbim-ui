<script lang="ts">
	...
	// Check if onboarding should be blocked
	const shouldBlockOnboarding = () => {
		return localStorage.getItem('blockOnboarding') === 'true';
	};
	...
	onMount(async () => {
		...
		if (($config?.features.auth_trusted_header ?? false) || $config?.features.auth === false) {
			...
			// Only show onboarding if it's not blocked
			onboarding = ($config?.onboarding ?? false) && !shouldBlockOnboarding();
		}
	});
</script>
